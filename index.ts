...
let currentQuestion: Question = questionPool[0]; // intermediate storage of the current question
let currentQuestionCount: number = -1; // index of the current question, initial value -1
...
function checkIfAnswerIsCorrect(answer: Answer): void {
  const answerCheckButtonElement = document.getElementById(
    answer.id.toString()
  );
